...
(function () {
    ...
    if (typeof Array.prototype.reduce !== 'function') {
        Array.prototype.reduce = function (callback, opt_initialValue) {
            /* jshint bitwise: false */  // allow bitwise oper in this scope
    ...
    // Removed the errors because they are bad UI for production (MK)
    // Though we should continue to show in dev.
    if (!window.console) {
        window.console = {
            error: function (msg) {
...
// Use this to interpret strings and not Date.parse(), which is
// not consistently implemented across browsers, and may even choose local vs.
// UTC capriciously based on browser and string format. See:
...
Date.createFromString = function (string, timezone) {
    ...
    // * YYYY-MM-DD hh:mm:ss
    // * YYYY-MM-DD
    // * Full precision ISO w/ timezone: http://www.w3.org/TR/NOTE-datetime

    var noTimezonePatterns = [
        // date only
        /^(\d\d\d\d)-(\d\d)-(\d\d)$/,
        // no fractional seconds
        /^(\d\d\d\d)-(\d\d)-(\d\d)[ T](\d\d):(\d\d):(\d\d)$/,
        // one to six decimals places
        /^(\d\d\d\d)-(\d\d)-(\d\d)[ T](\d\d):(\d\d):(\d\d)\.(\d{1,6})$/
    ];
    // http://www.w3.org/TR/NOTE-datetime
    var ISOPattern = /^(\d{4})-([01]\d)-([0-3]\d)T([0-2]\d):([0-5]\d):([0-5]\d)\.(\d+)([+-][0-2]\d:[0-5]\d|Z)$/;

    // If using a fully-precise ISO format w/ timezone, we can rely on all
    // browser implementations to parse it correctly.
    if (ISOPattern.test(string)) {
        return new Date(string);
    }

    // The timezone parameter must be either 'UTC' or 'local' if not using
    // a fully-precise ISO format.
    if (timezone !== 'local' && timezone !== 'UTC') {
        throw new Error("Timezone must be 'local' or 'UTC'. Got " + timezone);
    }

    var matches;
    forEach(noTimezonePatterns, function (p) {
...
var util = (function () {
    ...
    util.defaults = function (kwargs, defaults) {
        forEach(defaults, function (k, v) {
    ...
    util.isDevelopment = function () {
        ...
        var match = loc.match(/^https?:\/\/www\.mindsetkit\.org/);
        // If no matches, .match returns null
        if (match === null) {
            if (window.debug) {
                window.console.warn(
                    "Detected non-production environment. This function " +
                    "expects the Mindset Kit to be hosted on " +
                    "www.mindsetkit.org");
            }
            return true;
        }
    ...
    util.queryString = function (key, value) {
        ...
        var queryDict = util.parseQueryString(window.location.search);
    ...
    util.parseQueryString = function (queryString) {
        ...
        var rawQueryDict = JSON.parse(jsonString, function (key, value) {
            // This function is used to traverse arbitrarily nested JSON
            // structures and transform values. Since this is a flat structure,
            // we only want to execute when we're at the root, which is when
            // the JSON "key" is an empty string.
            return key === '' ? value : decodeURIComponent(value);
        });
        // Query parameter keys may need unescaping.
        var queryDict = {};
        forEach(rawQueryDict, function (key, value) {
            queryDict[decodeURIComponent(key)] = value;
        });
        return queryDict;
    };
    ...
    // Displays date nicely, assumes UTC if no timezone specified.
    // bmh 2013
    //
    // e.g. 2013-01-01 -> "X months ago"
    //
    // See: http://stackoverflow.com/a/6207162/431079
    util.prettyDate = function (date_str) {
        ...
        var then = Date.createFromString(date_str, 'UTC');
        var now = new Date();
        ...
        // If none of the known formats apply, default to the native, locale-
        // specific string.
